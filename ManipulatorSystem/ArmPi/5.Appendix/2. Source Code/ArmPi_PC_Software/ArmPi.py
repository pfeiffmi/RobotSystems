...
class MainWindow(QtWidgets.QWidget, Ui_Form):
    def __init__(self):
        ...
        self.tabWidget.setCurrentIndex(0)  # set the default label as the first label 
        self.tableWidget.setSelectionBehavior(QAbstractItemView.SelectRows)  # set to select the entire row, if not set the default selected cell
        ...
        # the slider synchronizes the corresponding text value, and the slider controls the rotation of the corresponding servo to bind to the valuechange function
        ...
        # tableWidget click to get positioning signal and icon_position function（add run icon）binding
        ...
        # bind the signal when the button of the edit action group is clicked with the button_editaction_clicked function
        ...
        # bind the click signal of the run and stop button with the button_runonline function
        ...
        # bind the signal of the button click that controls the action to the action_control_clicked function
...
#     
    ...
    def closeEvent(self, e):        
        result = QMessageBox.question(self,
                                    "close window messageBox reminder",
                                    "if to exit?",
    ...
    # the slider synchronizes the corresponding text box value, and the slider controls the rotation of the corresponding servo
    ...
    # select the label status on the tab to obtain the corresponding servo value
    ...
    # add a row data function to tableWidget table
    ...
    # add run icon button in the positioning line
    ...
    # edit action group button click event
    def button_editaction_clicked(self, name):
        ...
        if name == 'servoPowerDown':
            ...
            r = requests.post(self.url, json = self.cmd).json()
            if self.chinese:
                self.message_From('power down success')
            else:
        ...
        if name == 'angularReadback':
            ...
            if r["result"][0]:                           
                if int(self.lineEdit_time.text()) < 20:
                    if self.chinese:
                        self.message_From('运行时间必须大于 20')
                    else:
                        self.message_From('Run time must greater than 20')
                    return
                RowCont = self.tableWidget.rowCount()
                self.tableWidget.insertRow(RowCont)    # add one line 
                self.tableWidget.selectRow(RowCont)    # position the last row selected row                      
        ...
        if name == 'delectAction':    # delete action
            if RowCont != 0:
                ...
                self.tableWidget.removeRow(item)  # detele selected row                
                self.label_TotalTime.setText(str((self.totalTime)/1000.0))
        if name == 'delectAllAction':
            result = self.message_delect('this operation will delete all actions in the list, whether to continue？')
            if result == 0:                              
        ...
        if name == 'insertAction':    # insert action
            ...
            self.tableWidget.insertRow(item)       # insert one row
                 ...
                 if self.Button_Run.text() == '运行' or self.Button_Run.text() == 'Run':
                    ...
                    if self.checkBox.isChecked():
                        for i in range(self.tableWidget.rowCount() - self.row):
                            ...
                            self.timer.start(int(s))       # set timing interval and start
    ...
    def button_flie_operate(self, name):
        try:            
            if name == 'openActionGroup':
                ...
                # open a file
                # parameter 1: set composite Controls: parameter 2:QFileDialog title
                # parameter 3: default open the directory, the "." dot indicates the program running directory, and / indicates the current drive letter root directory
                # parameter 4: the file extension filter of the dialog box, for example, using Image files(*.jpg *.gif) means that only .jpg or .gif files can be displayed
                # set multiple file extension filtering, separated by double quotes; "All Files(*);;PDF Files(*.pdf);;Text Files(*.txt)"
                path = openfile[0]
                try:
                    if path != '':
                ...
                if r["result"][0]:
                    ...
                    for i in range(len(r["result"][1])):
                        if r["result"][1][i] == 999:
                            dev.append(0)
                            id += ' ' + 'id' + str(i + 1)
                        elif r["result"][1][i] > 125:  # negative number
            ...
            if name == 'tandemActionGroup':
                ...
                # Open a single file
                # Parameter 1: Set the parent component; Parameter 2: QFileDialog title
                # Parameter 3: The directory opened by default, the "." dot indicates the program running directory, and / indicates the current drive letter root directory
                # Parameter 4: The file extension filter of the dialog box, for example, using Image files(*.jpg *.gif) means that only .jpg or .gif files can be displayed
                # Set multiple file extension filtering, separated by double quotes; "All Files(*);;PDF Files(*.pdf);;Text Files(*.txt)"
                path = openfile[0]
                try:
    ...
    # control action group icon click event
    ...
    def getAreaMaxContour(self,contours) :
            ...
            for c in contours :
                contour_area_temp = math.fabs(cv2.contourArea(c)) # calculated area
                if contour_area_temp > contour_area_max : # if the new area is larger than the largest area in history, set the new area to the largest area in history
                    contour_area_max = contour_area_temp
                    if contour_area_temp > 10: # only the new historical maximum area is greater than 10, is the effective maximum area
                                               # remove too small contour
                        area_max_contour = c

            return area_max_contour # return the maximum area, or none if there is none
    ...
    def show_image(self):
        if self.camera_opened:
            ...
            if ret:
                ...
                mask = cv2.inRange(frame_lab, (self.L_Min, self.A_Min, self.B_Min), (self.L_Max, self.A_Max, self.B_Max))# perform bit operations on the original image and mask
