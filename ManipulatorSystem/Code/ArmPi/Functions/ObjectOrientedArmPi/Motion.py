...
class Motion():
    ...
    def run_sort(self, color_of_interest, position_vector, speed=1.0):
        ...
        # phase 8 - more slowly move cube to its home position
        servos, alpha, movetime_ms = self.AK.setPitchRangeMoving(
    ...
    def run_stack(self, stack_index, position_vector, speed=1.0):
        # ensure that the color or position vector is not 'None'
        if((stack_index >= 3) or (stack_index < 0) or (position_vector is None)):
            return
        # get the individual measures from the position vector
        world_X, world_Y = np.array(position_vector[0]) + self.offset_world_xy
        width, height = position_vector[1]
        ...
        stack_home_X, stack_home_Y, stack_home_Z = self.home_coordinate["stack"]
        stack_home_Z = stack_home_Z[stack_index]
        ...
        # phase 5 - move arm with cube 12 cm above the color's home position
        servos, alpha, movetime_ms = self.AK.setPitchRangeMoving(
            coordinate_data = (stack_home_X, stack_home_Y, 12), 
            alpha = -90, 
            alpha1 = -90, 
            alpha2 = 0,
            movetime = int(1500/speed)
        ...
        # phase 6 - adjust the claw's hand angle to line up with the home base
        servo2_angle = Transform.getAngle(stack_home_X, stack_home_Y, -90)
        Board.setBusServoPulse(2, servo2_angle, int(500/speed))
        time.sleep(0.5/speed)

        # phase 7 - move arm 3 inches above normal cube home postion
        servos, alpha, movetime_ms = self.AK.setPitchRangeMoving(
            coordinate_data = (stack_home_X, stack_home_Y, stack_home_Z+3), 
            alpha = -90, 
            alpha1 = -90, 
            alpha2 = 0, 
            movetime = int(500/speed)
        ...
        # phase 8 - more slowly move cube to its home position
        servos, alpha, movetime_ms = self.AK.setPitchRangeMoving(
            coordinate_data = (stack_home_X, stack_home_Y, stack_home_Z), 
            alpha = -90, 
            alpha1 = -90, 
            alpha2 = 0, 
            movetime = int(1000/speed)
