...
class Interpreter():
    def __init__(self, sensitivity=1.0, is_dark_line=True):
        # class variables
    ...
    def interpret_sensor_reading_discrete(self, sensor_reading, threshold=20):
        ...
        avg_floor_reading = np.avg(sensor_reading[floor_index])
        floor_line_difference = np.abs(line_reading - avg_floor_reading)
        if(floor_line_difference >= (threshold/self.sensitivity)):
            # if difference exceeds threshold
            return(line_index-1)
        else:
            # otherwise return no need to change angle
            return(0)
    ...
    def get_turn_proportion(self, avg_difference, scaling_function, threshold):
        ...
        if(scaling_function == "cubic"):
            ...
            x = avg_difference/np.power(threshold, 3)
            ...
            # threshold is where the value is 1
            x = avg_difference/np.power(threshold, 2)
    ...
    def interpret_sensor_reading_proportional(self, sensor_reading, scaling_function="linear", threshold=25):
        left_avg = np.avg(sensor_reading[0:2])
        right_avg = np.avg(sensor_reading[1:3])
