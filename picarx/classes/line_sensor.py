...
class LineSensor():
    ...
    def read_vision_data(self):
        ...
        _, self.image = cv2.threshold(self.image, thresh=150, maxval=255, type=cv2.THRESH_BINARY_INV)
        self.image = self.image/255.0

        num_bot_rows = int(0.05*self.image_dims[0])
        data = np.mean(self.image[(self.image_dims[0]-num_bot_rows):self.image_dims[0], :], axis=0)
